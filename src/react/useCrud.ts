...
export function useCrud<T extends Object>(props: Props<T>) {
  ...
  function postData(method: Methods) {
...
function createEditBody<T extends object>(
  ...
  if (init) {
    return edit as Edit<T>;
  }
  const obj: Partial<Edit<T>> = edit;
  ...
  return obj as Edit<T>;
}

function resetEdit<T extends object>(edit: Edit<T>) {
...
function setBody<T extends object>(method: Methods, state: State<T>) {
  let body;
...
function handleSuccess<T>(method: Methods, functions: Functions<T>, res: T) {
